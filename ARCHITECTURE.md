...
> Guia técnico detalhado da arquitetura, motor de inferência, modelo de dados e decisões de design do Atlas4Me — jogo interativo de dedução geográfica global sustentado por um sistema de inferência baseado em Entropia de Shannon.
...
O Atlas4Me é um jogo interativo de adivinhação de países com escopo **global** — 36 países distribuídos por múltiplos continentes. O usuário pensa em um país e o sistema tenta identificá-lo através de perguntas binárias (SIM/NÃO), exibindo em tempo real os candidatos restantes num **globo 3D interativo**. A experiência é similar ao Akinator, mas com foco em geografia mundial.

**Dimensão Técnica (mecanismo interno):**
O sistema opera como um **motor de inferência determinístico** baseado em **Teoria da Informação**. A cada rodada, o sistema calcula o **Ganho de Informação** de cada uma das 60 perguntas disponíveis usando a **Entropia de Shannon** e seleciona a pergunta que mais divide o espaço de hipóteses — reduzindo o conjunto de candidatos de forma ótima. Toda a operação ocorre **100% em memória RAM** via `KnowledgeBaseCache`, sem nenhuma query SQL durante o jogo.

### Funcionamento Resumido

```
1. USUÁRIO   →  pensa em um dos 36 países (não revela)
2. SISTEMA   →  KnowledgeBaseCache carrega matriz 36×60 da RAM na startup
...
5. GLOBO 3D  →  GameGlobe.jsx atualiza marcadores com os validIsoCodes
6. USUÁRIO   →  responde SIM ou NÃO
7. SISTEMA   →  InferenceEngine.filterCandidates() — interseção de HashSets em O(n)
8. SISTEMA   →  repete até |candidatos| ≤ 1 → status = GUESSING
9. USUÁRIO   →  confirma ou nega; ao final revela o país
10. SISTEMA  →  GameService calcula pontuação e registra no histórico
```
...
│  │  │  KnowledgeBaseCache (RAM: 36×60 matriz)         │   │ │
...
│  └──────────────────────┬──────────────────────────────────┘ │
└─────────────────────────┼────────────────────────────────────┘
                          │ JDBC + Flyway Migrations (V1–V4)
                          │ @PostConstruct → carrega RAM na startup
                          ▼
          ┌───────────────────────────────────────┐
          │         DATABASE (MySQL 8.0)           │
          │  36 países + 60 perguntas             │
          │  country_features (~2160 registros)   │
...
O ponto central da arquitetura. A lógica do motor foi extraída para um pacote dedicado com três componentes, cada um com responsabilidade única.
...
Carregado uma única vez no `@PostConstruct`, transforma a tabela `country_features` do banco em **estruturas de acesso rápido**:
...
```
questionToTrueCountries.get(1L)  → {BR, AR, CO, ...}   // países que falam Espanhol
questionToFalseCountries.get(1L) → {PT, GY, SR, ...}   // países que não falam Espanhol
```
...
Usado como desempate quando duas perguntas têm idêntico Ganho de Informação.
...
Para cada pergunta ainda não feita:
  ...
  6. Empate total: sorteio aleatório (variabilidade de partidas)
...
### `validIsoCodes` — O elo com o Globo 3D

Ao selecionar a próxima pergunta, o `GameService` também calcula quais países do conjunto de candidatos responderiam **SIM** àquela pergunta, e os devolve como `validIsoCodes` no `QuestionResponse`:
...
return new QuestionResponse(bestQ.getId(), bestQ.getText(), bestQ.getCategory(),
    bestQ.getHelperImageUrl(), validIsoCodes);
```

O frontend usa este array para atualizar os marcadores do `GameGlobe` em tempo real.
...
GameService.submitAnswer()
    ...
    │      ├─ universo inicial = knowledgeBaseCache.getCountryQuestionMatrix().keySet()
    │      ├─ for each attempt: candidateIds.retainAll(compatible)  ← índice invertido
    │      ├─ removeAll(rejectedIds)
    │      └─ countryRepository.findAllById(candidateIds)  ← 1 única query ao banco
    │
    ...
    │
    └─ 4. buildGameResponse()        → monta DTO de resposta unificado
```
...
│     version            │◄─── Optimistic Locking (@Version JPA)
│     started_at         │
...
**Volume de dados — V3/V4 após expansão global:**

| Entidade | Antes (S. America) | Depois (Global) |
|---|---|---|
| `countries` | 13 | **36** |
| `questions` | 16 | **60** |
| `country_features` | ~208 | **~2.160** |
| Migrations Flyway | V1–V2 | **V1–V4** |

### Explicação das Entidades

#### `CountryFeature` — A Base de Conhecimento
**O núcleo do sistema.** Matriz `País × Pergunta → Resposta booleana`. O `KnowledgeBaseCache` transforma essa tabela em índices invertidos na RAM na startup.

#### `Country`
Agora inclui campo `continent` (além de `latitude`, `longitude` e `iso_code`), suportando países de múltiplos continentes.

#### `GameSession`
Controla UMA sessão de inferência do início ao fim. `user_id` pode ser `NULL` (visitante). `target_country_id` é o país que o sistema sorteia internamente — invisível ao usuário durante o jogo. Campo `version` habilita Optimistic Locking.
...
Se ainda empatados: sorteio aleatório (variabilidade de partidas)
```
...
public Long selectBestQuestion(GameState state) {
    ...
    double currentEntropy = Math.log(total) / Math.log(2); // H(S) = log₂(|S|)
    ...
    for (Long questionId : cache.getAllQuestionIds()) {
        ...
        } else if (ig == maxIG && priority == bestPriority && Math.random() > 0.5) {
            bestQuestionId = questionId; // sorteio para variar partidas
        }
    }
...
GameService.startNewGame():
  ...
  3. selectNextQuestion():
     ├─ getRemainingCountries() → todos os 36 (sem tentativas ainda)
     ├─ new GameState(allIds, emptySet)
     ├─ inferenceEngine.selectBestQuestion(state) → 1ª pergunta por IG máximo
     └─ calcula validIsoCodes para a pergunta selecionada
  4. Retorna GameResponse com todos 36 países + primeira pergunta + validIsoCodes
```
...
GameService.submitAnswer():
  ...
  2. getRemainingCountries():
     ├─ universo inicial do cache (sem query ao banco)
     ├─ for each attempt: retainAll(índice invertido)  ← RAM pura
     ├─ removeAll(rejectedIds)
     └─ countryRepository.findAllById(ids)  ← 1 query ao banco
  3. if empty           → HUMAN_WON (contradição nas respostas)
     if size == 1       → GUESSING (convergiu!)
     else               → selectNextQuestion() + updateScore(-2)
                          → nextQuestion inclui validIsoCodes para o GameGlobe
```
...
POST /api/games/guess-feedback { gameId: 42, correct: true }
  → ROBOT_WON, score += 20

POST /api/games/guess-feedback { gameId: 42, correct: false }
...
PÚBLICO:   POST /api/games/start, answer, deny, confirm, guess-feedback, reveal
...
### 2. ✅ `Set<Long>` + índices invertidos (escalável para N países)
...
### 3. ✅ `validIsoCodes` retornado por pergunta

Ao invés de o frontend recalcular quais países são candidatos, o backend devolve diretamente os ISO codes dos países que responderiam SIM à próxima pergunta. O `GameGlobe` usa este dado diretamente como prop, eliminando qualquer lógica duplicada no cliente.

---

### 4. ✅ `CountryFeature` — Base de Conhecimento Normalizada
...
### 5. ✅ `GameStatus` enum — Múltiplos Estados

Enum com 7 estados claros: `IN_PROGRESS`, `GUESSING`, `WAITING_FOR_REVEAL`, `ROBOT_WON`, `HUMAN_WON`, `GAVE_UP`, `FINISHED_REVEALED`.
...
Suite de testes unitários cobrindo o motor de inferência com Mockito:
...
|---|---|---|
| V1 | `V1__create_table.sql` | Criação de todas as 7 tabelas (com campo `continent` em `countries`) |
| V2 | `V2__insert_initial_data.sql` | Dados iniciais do projeto |
| V3 | `V3__insert_world_countries.sql` | **36 países do mundo inteiro** + coordenadas |
| V4 | `V4__insert_more_questions.sql` | **60 perguntas** estratégicas + gabarito `country_features` completo |
